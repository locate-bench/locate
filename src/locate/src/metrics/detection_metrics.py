...
class ActionDetectionEvaluator(object):
    def __init__(self, dataset, groundtruth, prediction, output_dir, tiou_thresholds=np.linspace(0.1, 0.9, 9), subset='testing', verbose=True):
    ...
    def _import_prediction(self, prediction):
        ...
        for p in prediction:
          for videoid, v in p.items():
            # print(v['segment'].shape,v['segment'].shape,v['label'],v['score'])
    ...
    def wrapper_compute_average_precision(self):
        ...
        print(self.activity_index.keys())
        # print(ap)
        # np.savetxt("ap.csv", ap, delimiter=",")
        np.savetxt(os.path.join(self.output_dir, 'ap.csv'), ap, delimiter=',')
        return ap
    ...
    def evaluate(self):
        ...
        evaluate_stat = {
            'mAP': self.average_mAP
        }
        for tIoU, mAP in zip(self.tiou_thresholds, self.mAP):
            evaluate_stat['{0:.02f}_mAP'.format(tIoU)] = mAP
