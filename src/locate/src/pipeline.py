...
# * Training
parser.add_argument('--resume',type=str,default='../activitygraph_transformer_deformable_st_v10/output/checkpoints_/20211101-225143_checkpoints_numqueries100_lr1e-5_lrdrop1500_dropout0_clipmaxnorm0_weightdecay0_posemblearned_lrjoiner1e-5_nheads4_nenclayers4_ndeclayers4_hdim256_sr1_batchsize2_nposembdict512_numinputs100/checkpoint.pth'
...
parser.add_argument('--mode', default='eval', type=str,
                    help="eval on Val Set / extract feature on Training Set (extract_feat_train)")
parser.add_argument('--save_samples_and_attention', default=False, type=str2bool,
                    help="whether to save samples and attention or not")
...
def main(args):
    ...
    if args.cuda and args.distributed:
        ...
        try:
            ...
            print('dataset_train is empty, using dataset_test instead')
            sampler_train = torch.utils.data.SequentialSampler(dataset_test)
    ...
    if args.mode == 'extract_feat_train':
        test_stats = evaluate(0, model, criterion, postprocessors, data_loader_train, args.output_dir, args.dataset,
                              device, position_embedding=args.position_embedding, vis=args.save_samples_and_attention)
    else:
        test_stats = evaluate(0, model, criterion, postprocessors, data_loader_test, args.output_dir, args.dataset,
                              device, position_embedding=args.position_embedding, vis=args.save_samples_and_attention)

    if args.output_dir and utils.is_main_process():
        with open(os.path.join(checkpoint_dir, 'log.json'), 'a') as f:
