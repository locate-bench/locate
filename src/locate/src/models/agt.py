...
class AGT(nn.Module):
    ...
    def __init__(self, joiner, transformer, dim_feedforward, num_classes, num_queries, aux_loss=True):
        ...
        # self.input_proj = nn.Conv1d(69, hidden_dim, kernel_size=1)
        self.input_proj = nn.Conv1d(dim_feedforward, hidden_dim, kernel_size=1)
    ...
    def forward(self, samples, mask, fixed_position_emb=None):
        ...
        if fixed_position_emb is not None:
            # print('fixed_position_emb.shape: ', fixed_position_emb.shape)  # fixed_position_emb.shape:  torch.Size([30, 100])
            src, pos = self.joiner(fixed_position_emb, mask)
            src = samples
            # print('pos.shape, src.shape: ', pos.shape, src.shape)  # pos.shape, src.shape:  torch.Size([30, 256, 100]) torch.Size([30, 256, 100])
        else:
            src, pos = self.joiner(samples, mask)
        hs, _, edge = self.transformer(self.input_proj(src), (mask==1), self.query_embed.weight,pos)

        outputs_class = self.class_embed(hs)  # (2, 500, 2)
        outputs_segments = F.relu(self.segments_embed(hs))
        # print('outputs_class[-1].shape: ', outputs_class[-1].shape)  # [2, 10, 61]
        # print('outputs_class.shape: ', outputs_class.shape)  # [4, 2, 10, 61]  4=num_heads
        # print('outputs_segments.shape: ', outputs_segments.shape)  # [4, 2, 10, 2]
        # print('outputs_segments[-1].shape: ', outputs_segments[-1].shape)  # [2, 10, 2]
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, classification_loss_type='focal', focal_alpha=0.25, focal_gamma=2):
    ...
    def loss_labels(self, outputs, targets, indices, num_segments, log=True):
        ...
        # print('0.1, target_classes_o: {}'.format(target_classes_o))
        # print('1. src_logits: {}'.format(src_logits))
        # print('1. target_classes: {}'.format(target_classes))  # (2, 500)
        if self.classification_loss_type in ['ce']:
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
        elif self.classification_loss_type in ['focal']:
            ...
            loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_segments, alpha=self.focal_alpha,
                                         gamma=self.focal_gamma) * src_logits.shape[1]
        else:
            raise NotImplementedError('classification_loss_type {} is not implemented.'.format(self.classification_loss_type))
    ...
    def forward(self, outputs, targets):
        ...
        # print('num_segments: {}, get_world_size(): {}'.format(num_segments, get_world_size()))  # 1
...
def build(args):
    ...
    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict, eos_coef=args.eos_coef, losses=losses,
                             classification_loss_type=args.classification_loss_type, focal_alpha=args.focal_alpha,
