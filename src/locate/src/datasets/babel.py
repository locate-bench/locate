...
import datasets.babel_utils as babel_utils
...
class ThumosDetection(Dataset):
    ...
    def __init__(self, vid_data_file, num_classes, actions_dict, features_root, sample_rate, split, out_size, position_embedding='learned'):
        ...
        if self.position_embedding == 'fixed':
            self.position_emb_path = os.path.join('/'.join(features_root.split('/')[:-1]), 'babel_Nv_100_csz_8_ch_st_idxs.npy')
            self.predefined_position_emb = np.load(self.position_emb_path)
        else:
            self.predefined_position_emb = None
    ...
    def __getitem__(self, idx):
        ...
        if self.position_embedding == 'fixed':
            # print('vid', vid)
            anno_idx = vid.split('_')[3].split('@')[-1]
            # print('anno_idx: ', anno_idx)
            fixed_position_emb = self.predefined_position_emb[int(anno_idx)]
            target['fixed_position_emb'] = torch.tensor(fixed_position_emb, dtype=torch.float32)
        return inp, target
    ...
    def read_data(self, vid_data_file, split):
        ...
        for k, v in data.items():
            ...
            if not os.path.exists(os.path.join(self.features_root, k + '.npy')):
                continue
...
class ThumosVideoData(object):
    ...
    def __call__(self, target):
        ...
        set_targets = {}
        # set_targets['feature_path'] = anno['feature_path']
        ...
        if feature.size(1) <= out_size:
            ...
            for i in range(initial_size):
                tile_idx[i] += 1
                if np.sum(tile_idx) == upsampled_size:
                    break
            ...
            if self.mode == 'training':
                if len(list(range(0 + offset, feature.size(1) - offset))) < out_size:
                    ...
                    for i in range(initial_size):
                        tile_idx[i] += 1
                        if np.sum(tile_idx) == out_size:
                            break

                    sample_idx = torch.tensor(list(range(0 + offset, feature.size(1) - offset))).repeat_interleave(
                        torch.LongTensor(tile_idx))
                ...
                feature_sampled = []
                for idx in fidx:
                    feature_sampled.append(
                        torch.mean(feature[:, torch.tensor(list(range(idx - offset, idx + offset)))], dim=1).unsqueeze(
                            1))
...
def build_babel_detection(mode, args):
    root = Path(args.data_root)
    assert root.exists(), 'args.data_root: {} does not exist.'.format(args.data_root)
    ...
    # mapping_file = '/home/jksun/Program/gtad/packages/activitygraph_transformer/data/thumos/action_mapping.txt'
    # print(mapping_file)
    file_ptr = open(mapping_file, 'r')
    actions = file_ptr.read().split('\n')[:-1]
    # print(actions) # ['0 walk']
    file_ptr.close()
    actions_dict = dict()
    for a in actions:
        # print(a.split())
        ...
        # actions_dict[a.split()[1]] = min(20, int(a.split()[0]))

    num_classes = len(actions_dict)
    assert num_classes == args.num_classes, \
        'num_classes {} != args.num_classes {}'.format(num_classes, args.num_classes)
    # check if the number of classes are determined correctly

    dataset = ThumosDetection(vid_data_file, args.num_classes, actions_dict, features_path, args.sample_rate, mode,
                              args.num_inputs, args.position_embedding)
